...
`cook`은 승인되고 `plate`로 계획된 numbered spec 전체를 실제 코드 변경으로 구현하는 지휘 skill입니다. 사용자는 `/vr:cook`으로 호출할 수 있습니다.

`cook`의 책임은 scope 결정이나 task 설계가 아니라 구현 지휘입니다. 제품 의도와 acceptance criteria는 `recipe`에서, 구현 접근과 task breakdown은 `plate`에서 이미 준비되어 있어야 합니다. 메인 에이전트는 recipe 전체를 관리하고, task 하나의 실제 실행은 `task-runner` agent에 맡긴 뒤 결과를 받아 통합합니다.

## 목표

- `Approved` 또는 `In Progress` spec과 plate 계획을 읽고 acceptance matrix를 만듭니다.
- plate가 작성한 phase order, wave order, task dependency, write scope, serial/parallel 가능성을 검증합니다.
- plate의 `Task 0` 실패 test 또는 executable acceptance check를 먼저 실행하게 합니다.
...
| `Plate 상태: Planned` 없음 | `plate`로 라우팅 |
| `구현 계획` 또는 `작업 목록` 없음 | `plate`로 라우팅 |
| task에 `Phase/Story/Wave` 없음 | `plate` 보강 필요 |
| task가 지정되지 않음 | recipe 전체 구현 모드 |
| task가 지정됨 | 해당 task만 실행하되 전체 acceptance 영향은 기록 |
| `Task 0` 없음 | 구현 전에 `plate` 보강 필요 |
...
여러 active spec이 있어도 `cook`은 한 번에 하나의 spec과 하나의 task lane만 구현합니다. 다른 active spec의 task나 acceptance를 같은 run이나 commit에 섞지 않습니다.

## 실행 흐름

1. `AGENTS.md`, active spec의 `recipe`/`plate` 섹션, `.agent/spec/design.md`, `.agent/commands.json`, 관련 ADR과 handoff를 읽습니다.
2. `git status --short`로 현재 변경을 확인합니다.
3. 모든 task의 `Covers`와 `Check`를 acceptance criteria와 검증 계획에 매핑합니다.
4. plate의 phase order, wave order, task dependency와 write scope를 기준으로 실행 계획을 검증합니다.
5. `Task 0`을 `task-runner`에게 맡깁니다.
6. 준비된 task를 phase와 wave 순서대로 맡깁니다. 기본은 linear이고, 같은 wave의 안전한 병렬 후보만 opt-in 병렬 실행합니다.
...
| Task | 이 acceptance를 구현하거나 검증하는 plate task |
| Check | task에 연결된 test/manual check |
| Status | pending, covered, blocked |
| Evidence | command, handoff, manual check 결과 |

`cook`은 `Setup -> Foundation -> US-### priority order -> Polish` phase 순서를 지킵니다. 앞 wave가 완료되고 check가 통과해야 다음 wave를 시작합니다.
...
병렬 구현은 사용자가 opt-in했고 같은 wave의 task들이 `Parallel: Yes`, dependency 없음, write scope disjoint 조건을 모두 만족할 때만 허용합니다. worker에게는 다른 작업자가 있을 수 있고, 서로의 변경을 되돌리지 말라고 명시해야 합니다.
...
grep -q 'wave order' plugins/vibe-recipe/skills/cook/SKILL.md
grep -q 'Cook Summary' plugins/vibe-recipe/skills/cook/SKILL.md
grep -q 'plate' plugins/vibe-recipe/skills/cook/SKILL.md
