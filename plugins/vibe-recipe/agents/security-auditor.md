...
description: taste 또는 fix에서 auth, secret, injection, unsafe IO, dependency, data-loss 같은 보안 위험을 구조적으로 점검해야 할 때 사용합니다.
...
- `fix/debug`에서 보안 의심 영역이 실패 원인에 포함될 때.
...
- audit 결과를 임의로 파일에 쓰지 않고 부모 `taste` 또는 `fix` 흐름에 반환합니다.
