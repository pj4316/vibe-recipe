...
- release set에 포함할 active spec이 1개 이상 있고, 각 spec folder의 `memory.md` 최신 `Taste Report` 섹션이 `Verdict: APPROVE`와 `Release readiness: Ready for Wrap`을 포함합니다. legacy repo라면 `.agent/spec/handoffs/NNNN-taste.md`를 read-only fallback으로만 사용합니다.
...
9. Commit: `chore(release): X.Y.Z` 제목과 release set의 모든 `Refs: .agent/spec/active/NNNN-<slug>/spec.md` footer를 함께 써서 commit을 만들고 `serve`로 넘길 release prep summary를 남깁니다.
...
release set 후보를 확정하기 전, blocked 상태, 그리고 wrap 완료 후 `serve`로 넘기기 전에는 최종 응답에 `templates/recommendation-block.md`와 같은 헤더를 포함합니다.

- `wrap` 후보 제시: 1순위는 현재 release set으로 진행, 차선은 spec 분할 또는 제외입니다.
- `wrap blocked`: 1순위는 missing taste, verify, version source, dirty tree 중 실제 blocker를 해소하는 skill, 차선은 release 연기입니다.
- `wrap done`: 1순위는 `serve` human gate, 차선은 release prep 검토 보류입니다.
