...
release branch를 준비하되 실제 release는 하지 않을 때 사용합니다. `wrap`은 version manifest와 `CHANGELOG.md`를 갱신하고 release prep commit을 만들지만, tag, push, deploy는 `serve`로 넘깁니다.

## 역할 구분

- `wrap`은 사용자가 호출할 수 있는 release-prep skill입니다.
- 입력 gate는 최신 `taste` verdict와 release 대상 commit range입니다.
- 출력은 version/changelog 변경과 `chore(release): X.Y.Z` commit입니다.
...
- version source와 changelog source를 찾았습니다. 없으면 어떤 파일을 release source로 둘지 확인 전에는 쓰지 않습니다.
...
- version manifest: 예를 들어 `package.json`, `pyproject.toml`, `Cargo.toml`, plugin manifest, app manifest.
- `CHANGELOG.md` 또는 프로젝트가 쓰는 changelog 파일.
...
- 여러 manifest version이 있으면 public release artifact에 해당하는 manifest만 갱신합니다. 어떤 manifest가 source인지 불명확하면 쓰기 전에 멈춥니다.
- prerelease, backport, hotfix line은 명시 요청이 있을 때만 처리합니다.

## Changelog 포맷

- 기존 `CHANGELOG.md`가 있으면 제목, 날짜 형식, 섹션 이름, bullet 스타일을 유지합니다.
- changelog가 없으면 `CHANGELOG.md`를 만들고 `# Changelog` 제목 아래 `## X.Y.Z - YYYY-MM-DD` 섹션을 추가합니다.
...
4. Changelog plan: 기존 changelog 포맷을 감지하거나 새 `CHANGELOG.md` 생성 포맷을 정합니다.
5. Preview: target version, 포함 commit, changelog section, 갱신 파일, 제외할 dirty file을 보여주고 override 기회를 줍니다.
6. Update: manifest version과 `CHANGELOG.md`만 갱신합니다.
7. Verify: `.agent/commands.json`의 `verify`를 실행합니다.
8. Commit: `chore(release): X.Y.Z` commit을 만들고 `serve`로 넘길 release prep summary를 남깁니다.

## 쓰기 범위

- 허용: release version manifest, `CHANGELOG.md`, release prep commit.
...
- changelog와 manifest version이 서로 다르면 commit하지 않습니다.
- changelog에 target version heading이 없으면 commit하지 않습니다.
- changelog에 raw diff, raw test log, internal handoff 내용이 들어가면 commit하지 않습니다.
