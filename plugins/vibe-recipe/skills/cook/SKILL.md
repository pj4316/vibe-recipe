...
- `Task 0`의 실패 test 또는 executable acceptance check가 먼저 수행됩니다.
...
1. Preflight: 기준 문서, command profile, branch, working tree, 이전 handoff를 확인합니다.
2. Matrix: acceptance criteria, tasks, `Check`, command를 연결한 acceptance matrix를 만듭니다.
3. Plan: task dependency, write scope, serial/parallel 가능성을 판단합니다.
...
- 다음 단계가 `taste`, `fix`, `recipe` 보강, 또는 추가 `cook` 중 하나로 분명합니다.
