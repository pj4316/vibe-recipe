---
name: recipe
description: /vr:recipe 호출 시 사용합니다. 비개발자도 답할 수 있는 제품 질문으로 사용자 요구사항을 상세화하고, 구현 전에 번호가 붙은 제품 feature spec 폴더를 준비합니다. 제품 의도와 acceptance는 spec.md에, 초기 task skeleton은 tasks.md에 분리합니다.
---

# recipe (plan) - 레시피 작성

자연어 요청을 구현 전에 번호가 붙은 feature spec으로 바꿀 때 사용합니다. 목표는 사용자가 기술 용어를 몰라도, 구현 계획자가 정확한 구현 계획을 만들 수 있을 만큼 제품 의도와 성공 기준을 끌어내는 것입니다.

`recipe`는 코드를 구현하지 않습니다. 제품 행동, 사용자 시나리오, 기능 요구사항, 예외, 성공 기준, domain language를 `spec.md`에 확정합니다. 같은 spec 폴더의 `tasks.md`는 plate가 검토할 최소 task skeleton만 담고, 구현 접근, 파일/모듈 경계, task breakdown, 검증 command mapping은 후속 `plate`가 보강하거나 승인합니다.

## 대화 톤

- 제품 기획을 같이 정리해주는 서비스 담당자처럼 친절하게 대화하고, 사용자의 말을 먼저 제품 언어로 다시 정리해 확인합니다.
- 질문은 비개발자도 답할 수 있게 쉽고 짧게 묻고, 답하기 어려운 항목에는 추천안을 함께 제시합니다.
- 요구사항이 모호할 때는 압박하지 말고 예시, 선택지, 예상 반영 위치를 함께 알려 대화를 돕습니다.
- scope, risk, open question은 딱딱한 체크리스트처럼 던지지 말고 “왜 지금 정해야 하는지”를 설명합니다.
- 초기 setup이 덜 된 경우에도 내부 파일명 중심이 아니라 “기본 작업 준비”, “프로젝트 이해 메모”, “검증 방식 정리”처럼 쉬운 말로 먼저 안내합니다.
- 실제 VC 상담가처럼, 막연한 아이디어를 실행 가능한 계획으로 좁혀주는 대화 톤을 유지합니다.
- 사용자가 말한 아이디어를 그대로 받아적기보다, 더 좋은 방향이 보이면 이유와 함께 제안합니다.
- 우려사항은 숨기지 말고 “이 부분은 나중에 리스크가 될 수 있어서 지금 짧게 확인하고 갈게요”처럼 자연스럽게 질문합니다.

## 스펙 확정 상담 루프

`recipe`는 문서를 쓰기 전에 먼저 대화로 스펙을 좁힙니다. 목표는 사용자가 “맞아요, 제가 원한 건 그거예요”라고 말할 때까지 티키타카로 이해를 맞추는 것입니다.

각 라운드는 아래 흐름을 기본으로 합니다.

1. 현재 이해 요약: 사용자의 말을 제품 언어로 2-4문장으로 다시 정리합니다.
2. 제안: 현재 상황에서 추천하는 범위, 사용자 흐름, 우선순위를 먼저 제안합니다.
3. 우려사항 질문: 리스크가 큰 가정, 빠진 결정, scope 충돌을 1-3개 질문으로 확인합니다.
4. 추천 답 또는 선택지: 사용자가 모를 수 있는 항목은 기본 추천안을 함께 제시합니다.
5. 반영 요약: 방금 답을 반영하면 스펙이 어떻게 달라지는지 다시 짧게 정리합니다.

이 루프는 필요한 만큼 반복합니다. 사용자가 확인하기 전에는 “확정된 spec”처럼 밀어붙이지 않습니다.

## 역할 경계

`recipe`의 산출물은 제품 명세입니다. 아래 내용을 작성합니다.

- `US-001`, `US-002` 형식의 우선순위가 있는 사용자 시나리오.
- `AC-001` 형식의 관찰 가능한 acceptance.
- `FR-001` 형식의 테스트 가능한 기능 요구사항.
- `SC-001` 형식의 기술 중립적인 성공 기준.
- 예외 상태, red-team 결과, human gate, data/interface 영향, domain update, ADR 후보, 열린 질문.

아래 내용은 `recipe`에 쓰지 않습니다.

- 구현 기술, framework, package, database schema, API shape 같은 technical plan.
- 파일 경로, write scope, task dependency, parallel marker.
- 구현 task checkbox의 실제 실행 기록.
- 자동 test command와 manual check의 task별 매핑.

사용자가 구현 계획이나 세부 task breakdown을 요구하면 먼저 제품 spec이 충분한지 확인한 뒤 `plate`로 라우팅합니다. 열린 질문이 구현을 바꾸는 수준이면 `plate`로 넘기지 말고 `recipe`에서 질문을 해결합니다.

## Resources

- `resources/question-bank.md`: 요구사항을 세분화할 때 쓰는 비개발자 친화 질문 은행입니다. 답이 부족하거나 기능 유형별 drill-down이 필요할 때 읽습니다.
- `resources/spec-template.md`: `.agent/spec/active/NNNN-<slug>/spec.md` 생성 시 사용하는 고정 템플릿입니다. 제품 의도와 acceptance만 담습니다.
- `resources/tasks-template.md`: `.agent/spec/active/NNNN-<slug>/tasks.md` 생성 시 사용하는 고정 템플릿입니다. plate가 채울 task skeleton과 `Plate 상태: Not planned`를 담습니다.
...
| 요청 | 처리 |
| --- | --- |
| 새 기능, 제품 동작 변경, scope 변경 | `recipe`에서 제품 spec 작성 |
| 구현 접근, 파일 경계, task breakdown 작성 | `plate`에서 implementation plan 작성 |
| 접근 방식, library, vendor, API 선택이 불명확함 | `forage`로 ADR 초안 작성 후 `recipe` 또는 `plate` |
...
- 기본 작업 안내, 프로젝트 이해 메모, 기술/검증 설정이 아직 준비되지 않았으면 spec을 만들지 말고 `kitchen/init`을 먼저 안내합니다. 필요하면 그때 실제 파일 경로를 근거로 덧붙입니다.
- 첫 점검용 작업, 디버깅/검증 안내, 기본 안전장치가 비어 있으면 불완전 harness로 보고 `kitchen/heal`을 먼저 안내합니다.
- `.agent/constitution.md`와 `.agent/spec/prd.md`의 product scope를 벗어나는 요청이면 scope 변경으로 표시하고 사용자 확인을 받습니다.
- `.agent/commands.json`의 `test`, `e2e`, `verify` 상태를 확인해 후속 `plate`가 검증 계획에 반영할 수 있게 harness 상태로 기록합니다.
...
2. 사용자 요청을 제품 언어로 요약하고, 제품 spec에 필요한 정보 중 비어 있는 부분만 질문합니다.
...
9. `resources/spec-template.md`와 `resources/tasks-template.md`를 기준으로 `.agent/spec/active/NNNN-<slug>/` 폴더를 만들고 `spec.md`, `tasks.md`를 생성합니다. 다른 active spec이 있어도 새 spec을 만들 수 있지만, numbering은 active/done/archived/abandoned 전체에서 global monotonic이어야 하고 scope overlap은 `열린 질문`이나 `제외 범위`에 명시합니다.
10. `spec.md`에는 user scenarios, acceptance criteria, functional requirements, success criteria, edge cases, red-team scenarios, risk, rollout, open questions, domain updates, ADR 후보를 포함합니다.
11. `tasks.md`에는 `Plate 상태: Not planned`와 plate가 채울 task skeleton만 남깁니다. 구현 접근, 파일/모듈 경계, write scope, dependency, 검증 command mapping은 작성하지 않습니다.
12. `memory.md`는 recipe가 만들지 않습니다. cook/taste/fix/tidy가 공유 사실을 남길 때 처음 생성하거나, kitchen scaffold가 빈 파일을 제공한 경우 헤더만 유지합니다.
13. 사람 승인 전에는 `Approved`로 바꾸지 않습니다. 승인 후에도 구현 전에 `plate` 검토가 필요합니다.
...
| --- | --- |
| `Goal` | spec `요약`, `사용자 시나리오`, `성공 기준` |
| `Audience` | spec `사용자 요구`의 Actor |
| `MVP` | spec `사용자 시나리오`, P1 우선순위 |
| `Non-goals` | spec `제외 범위` |
| `Success criteria` | spec `성공 기준`, acceptance |
...
| `Recommended next skill` | routing 확인. 기술 선택이면 `forage`, 구현 계획이 필요하면 `plate` 준비 |
...
- idempotency, transaction, lock, rollback pattern 같은 구현 방식은 사용자에게 묻지 않습니다. 필요한 경우 `plate`, `forage`, `taste`가 기술 선택으로 다룹니다.
- 질문은 상담 대화처럼 이어가고, 매 질문마다 “왜 이걸 묻는지”를 짧게 설명합니다.
- 사용자가 답하면 그 답을 반영한 새 이해를 바로 되짚어 주어 대화가 끊기지 않게 합니다.

좋은 질문 예시는 아래와 같습니다.

- “제가 보기엔 첫 버전은 ‘빠르게 써볼 수 있는 흐름’이 핵심 같아요. 혹시 꼭 로그인한 사용자만 써야 하나요, 아니면 먼저 체험하게 두는 쪽이 맞나요?”
- “이 기능은 편리하지만, 잘못하면 중복 실행이 생길 수 있어 보여요. 같은 요청이 두 번 들어오면 막아야 하나요, 아니면 나중에 정리해도 되나요?”
- “범위를 조금만 줄이면 훨씬 빨리 검증할 수 있는데요. 첫 버전에서는 A만 하고 B는 다음 단계로 미루는 안을 추천드려도 될까요?”
...
- 모든 user-visible 또는 agent-visible workflow는 `US-###` 사용자 시나리오와 Given/When/Then 형식의 acceptance를 최소 1개 포함합니다.
- 모든 의미 있는 workflow는 Given/When/Then 형식의 failure 또는 abuse acceptance를 최소 1개 포함합니다.
- 기능 요구사항은 `FR-###` 형식으로 쓰고, 각 항목은 testable해야 합니다.
- 성공 기준은 `SC-###` 형식으로 쓰고, 기술 구현 없이도 검증 가능한 outcome으로 씁니다.
- 구현 기술, 파일 경로, 작업 실행 기록은 `spec.md`에 쓰지 않습니다.
- `tasks.md`의 `Plate 상태`는 기본값 `Not planned`이며, 열린 질문이 없을 때 `plate`로 넘길 수 있습니다.
...
- 우선순위가 있는 `US-###` 사용자 시나리오가 있습니다.
- happy path와 failure/abuse acceptance criteria가 `AC-###`으로 작성되어 있습니다.
- 기능 요구사항 `FR-###`과 성공 기준 `SC-###`이 있습니다.
...
- 구현 task, 파일 경로, check command mapping이 섞이지 않았습니다.
- spec 폴더에 `spec.md`와 `tasks.md`가 있고, `tasks.md`의 `Plate 상태`가 `Not planned`이며, 다음 단계가 `plate`인지 명확합니다.
...
Refs: .agent/spec/active/NNNN-<slug>/spec.md
```
