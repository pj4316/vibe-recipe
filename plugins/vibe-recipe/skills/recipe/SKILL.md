---
name: recipe
description: /vr:recipe 호출 시 사용합니다. 비개발자도 답할 수 있는 제품 질문으로 사용자 요구사항을 상세화하고, 구현 전에 번호가 붙은 제품 feature spec을 준비합니다. 구현 접근, 파일 경계, task breakdown은 plate가 담당합니다.
---
...
`recipe`는 코드를 구현하지 않고, 구현 task도 작성하지 않습니다. 제품 행동, 사용자 시나리오, 기능 요구사항, 예외, 성공 기준, domain language를 확정합니다. 구현 접근, 파일/모듈 경계, task breakdown, 검증 command mapping은 후속 `plate`가 작성합니다.
...
- `Task 0`, 구현 task checkbox, 작업 실행 기록.
- 자동 test command와 manual check의 task별 매핑.

사용자가 구현 계획이나 task를 요구하면 먼저 제품 spec이 충분한지 확인한 뒤 `plate`로 라우팅합니다. 열린 질문이 구현을 바꾸는 수준이면 `plate`로 넘기지 말고 `recipe`에서 질문을 해결합니다.
...
9. `resources/spec-template.md`를 기준으로 `.agent/spec/active/NNNN-<slug>.md`를 `Status: Draft`로 생성합니다. 다른 active spec이 있어도 새 spec을 만들 수 있지만, numbering은 global monotonic이어야 하고 scope overlap은 `열린 질문`이나 `제외 범위`에 명시합니다.
10. user scenarios, acceptance criteria, functional requirements, success criteria, edge cases, red-team scenarios, risk, rollout, open questions, domain updates, ADR 후보를 포함합니다.
11. `Plate 상태`를 `Not planned`로 남기고, 구현 계획과 task breakdown은 작성하지 않습니다.
12. 사람 승인 전에는 `Approved`로 바꾸지 않습니다. 승인 후에도 구현 전에 `plate`가 필요합니다.
...
- 구현 기술, 파일 경로, task checkbox, `Task 0`, 작업 실행 기록은 쓰지 않습니다.
- `Plate 상태`는 기본값 `Not planned`이며, 열린 질문이 없을 때 `plate`로 넘길 수 있습니다.
