...
| `cook/dev` | plated active spec, `.agent/spec/design.md`, `.agent/commands.json` | cook summary, task-runner handoff, acceptance matrix |
...
새 기능은 `recipe`와 `plate` 없이 `cook`으로 건너뛰지 않습니다. `plate` task는 `Phase`, `Story`, `Wave`, `Covers`, `Write scope`, `Dependency`, `Parallel`, `Check`를 가져야 합니다. `cook`, `fix`, `tidy`는 실행한 command와 결과를 handoff 또는 관련 spec에 남깁니다. `taste`, `wrap`, `serve`는 `verify`가 없거나 실패하면 release를 진행하지 않습니다. 여러 active spec은 허용하지만, `cook`은 한 번에 하나의 spec/task lane만 구현하고 `wrap`은 `Ready for Wrap` spec만 release set으로 묶습니다.
...
| 경로 | 목적 |
| --- | --- |
| `.agent/spec/active/` | draft, approved, in-progress, ready-for-wrap spec |
