---
name: kitchen
description: /vr:kitchen 호출 시 사용합니다. 새 프로젝트 또는 기존 서비스에 vibe-recipe orchestration harness를 설치/도입/복구합니다. AGENTS.md, hooks, .agent, command profile을 구성해 recipe -> plate -> cook -> taste workflow가 재현 가능하게 동작해야 할 때 사용합니다.
---
...
- `.agent/`: spec folder, command profile, runbook, domain language, memory.
...
- `resources/design-system.md`: foundations-first design system 플레이북입니다. token hierarchy, focus visible을 포함한 accessibility checklist, composition pattern, governance까지 한 번에 담아 UI 정책을 바로 결정할 수 있게 합니다.
...
- 아키텍처 문서는 감지된 entry point, source/test/config 경로, 외부 interface, data store, deploy surface를 “감지된 사실 + 미확정 항목”으로 기록합니다.
...
- 모든 의미 있는 변경은 numbered spec folder에서 시작합니다.
- `.agent/constitution.md`는 초기화 이후 human-only입니다.
- `recipe`는 `.agent/spec/active/NNNN-<slug>/spec.md`에 제품 요구사항과 acceptance를 확정하고, 같은 폴더의 `tasks.md`에 plate용 skeleton을 둡니다.
- `plate`는 `tasks.md`를 검토·보강해 구현 계획과 task breakdown을 확정합니다.
- `cook`은 approved/plated recipe 전체 구현을 지휘합니다.
...
- `.agent/spec/INDEX.md`와 `.agent/spec/handoffs/INDEX.md`는 만들지 않습니다. `peek`과 librarian은 active/done/archived/abandoned 폴더를 on-read로 스캔합니다.
...
| `.agent/spec/active/0001-health-check/spec.md` | harness rehearsal용 create only. |
| `.agent/spec/active/0001-health-check/tasks.md` | health-check spec의 plate/cook rehearsal task skeleton. |
| `.agent/spec/active/0001-health-check/memory.md` | cook/taste/fix/tidy handoff와 공유 사실 누적 로그. |
...
| `.agent/memory/*` | create only. librarian이 이후 관리. |
...
필요한 directory도 함께 만듭니다: `.agent/spec/{active,done,archived,abandoned}`, `.agent/autopilot`, `.agent/wiki/decisions`, `.agent/memory/{topics,handoffs}`, `.agent/runbooks`, `.agent/setup`, `.claude`.

## Heal: legacy spec migration

기존 `.agent/spec/active/NNNN-*.md` 단일 파일과 `.agent/spec/handoffs/`가 있으면 `heal` 모드에서 새 spec folder 구조로 옮기는 preview를 먼저 제시합니다.

1. 감지: `.agent/spec/active/NNNN-*.md`, `.agent/spec/handoffs/NNNN-*.md`, `.agent/spec/INDEX.md`, `.agent/spec/handoffs/INDEX.md` 존재 여부를 확인합니다.
2. preview: `node plugins/vibe-recipe/scripts/state-migrate.mjs .agent/spec`를 dry-run으로 실행하거나 동일한 이동 표를 만들어 사용자에게 보여줍니다. preview에는 생성될 `spec.md`, `tasks.md`, `memory.md`, 삭제될 legacy index/handoffs 경로가 포함되어야 합니다.
3. 승인: 데이터 이동과 legacy index 삭제는 hard-to-reverse이므로 사용자가 명시 승인하기 전에는 apply하지 않습니다.
4. 적용: 승인 후 `node plugins/vibe-recipe/scripts/state-migrate.mjs .agent/spec --apply`를 실행합니다. 각 legacy spec은 `.agent/spec/active/NNNN-<slug>/{spec.md,tasks.md,memory.md}`로 변환됩니다. 실패하면 같은 명령을 재실행해 완료 가능한 단위로 설계된 migration script를 기준으로 복구합니다.
5. 검증: `peek`으로 active spec 목록과 최신 handoff가 동일하게 보이는지 확인하고, `node plugins/vibe-recipe/scripts/state-migrate.mjs .agent/spec` dry-run이 더 이상 legacy move를 제안하지 않는지 확인합니다. 실패하면 원본을 기준으로 수동 복구 경로를 recommendation block으로 제시합니다.
...
- `.agent/wiki/domain.md`, `.agent/memory/gotchas.md`, health-check spec folder, runbooks가 있습니다.
