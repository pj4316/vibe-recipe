...
- `taste`와 review subagent는 제품 코드를 수정하지 않습니다. 허용되는 쓰기는 현재 spec folder의 `memory.md`에 추가하는 taste report뿐입니다.
- `APPROVE` verdict는 active spec 본문을 `Done`으로 바꾸지 않습니다. 대신 report에 `Release readiness: Ready for Wrap`을 남겨 `wrap`이 release set 후보로 찾을 수 있게 합니다.
...
- `taste` 완료 후 메인 응답은 verdict, blocker, coverage gap, next loop만 요약하고 상세 원자료는 현재 spec folder의 `memory.md` taste 섹션을 가리킵니다.
...
3. Coverage: acceptance matrix, task `Covers`, task `Check`별 evidence를 대조합니다.
...
- task checkbox 완료 여부만으로 coverage를 인정하지 않습니다. 각 `US/AC/FR/SC`는 `Covers`와 command/manual evidence가 연결되어야 합니다.
- phase/wave 순서 위반, unmapped task, `Parallel: Yes`인데 write scope가 겹치는 task는 coverage gap 또는 blocker로 기록합니다.
...
`taste`는 review report를 현재 spec folder의 `memory.md`에 append-only로 남깁니다. 이 섹션은 `peek`, `wrap`, `serve`가 같은 spec의 최신 verdict와 release readiness를 재사용할 수 있는 source입니다.

제품 코드, test, generated artifact, active spec 본문은 수정하지 않습니다. spec 자체가 틀렸다면 report에 `recipe` escalation을 남깁니다.

`APPROVE`일 때만 아래 release readiness 값을 사용합니다.

- `Release readiness: Ready for Wrap`: acceptance와 verification evidence가 release prep 후보로 충분합니다.
- `Release readiness: Not Ready`: `APPROVE`가 아닌 verdict이거나 human gate, missing verify, unresolved blocker 때문에 release set에 넣을 수 없습니다.
...
Release readiness: Ready for Wrap / Not Ready
Reason:
Source spec:
Diff scope:
Handoff source:
...
verdict를 낸 직후 report와 최종 응답에 `templates/recommendation-block.md`와 같은 헤더를 포함합니다.

- `taste APPROVE`: 1순위는 `wrap` 또는 merge 전 human review, 차선은 follow-up을 분리한 뒤 `taste` 재실행입니다.
- `taste REQUEST_CHANGES`: 1순위는 `fix`, `cook`, `plate` 중 findings를 가장 직접 줄이는 skill, 차선은 scope를 줄여 재검토하는 경로입니다.
- `taste BLOCK`: 1순위는 blocker 원인을 해소하는 skill 또는 사용자 결정, 차선은 release/merge 중단입니다.
...
- verdict가 `APPROVE`이면 report에 `Release readiness: Ready for Wrap`을 남기고, version/changelog 준비가 필요하면 `wrap`으로 보냅니다.
...
- `APPROVE` 후에도 active spec은 그대로 둡니다. release set이 `serve`를 통과한 뒤에만 포함 spec을 `done/`으로 이동합니다.
