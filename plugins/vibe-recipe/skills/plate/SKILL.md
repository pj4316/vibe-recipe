---
name: plate
description: /vr:plate 호출 시 사용합니다. 번호가 붙은 recipe spec folder를 읽고 cook 전에 필요한 구현 계획, user story별 phase/wave task breakdown, 검증 매핑을 tasks.md에서 검토·보강합니다.
---

# plate (implementation plan) - 플레이팅

`plate`는 `recipe`가 만든 spec folder를 구현 가능한 계획으로 검토·보강하는 스킬입니다. 코드를 구현하지 않습니다. 제품 의도는 `spec.md`에서 바꾸지 않고, `cook`이 task-runner에게 넘길 수 있는 implementation plan과 task list를 `tasks.md`에 확정합니다.

## 역할 경계

`plate`가 `tasks.md`에 작성하거나 보강하는 내용:
...
- `spec.md`의 제품 scope, user story, acceptance criteria를 직접 수정하지 않습니다. 보강이 필요하면 `recipe`로 되돌립니다.
...
- `.agent/spec/active/NNNN-<slug>/spec.md`와 `tasks.md`가 있습니다.
- `spec.md`에는 `US-###`, `AC-###`, `FR-###`, `SC-###` 중 구현에 필요한 추적 ID가 있습니다.
- `tasks.md`에는 `Plate 상태: Not planned` 또는 보강 중인 작업 계획이 있습니다.
...
1. Preflight: active spec folder의 `spec.md`, `tasks.md`, command profile, domain/design docs, 관련 ADR을 읽습니다.
...
4. Plan sections: `tasks.md`에 `## 구현 계획`, `## 작업 목록`, `## 실행 순서`, `## 검증 계획`을 추가하거나 갱신합니다.
...
- W00: Task 0
- W01: Task 1, Task 2
- Parallel policy: 같은 wave 안에서도 `Parallel: Yes`, dependency 충족, write scope disjoint인 task만 worker 병렬 후보.
...
- recipe 작성 행위 자체인 “spec 폴더 생성”, “index 등록”을 구현 task로 쓰지 않습니다. 단, 실제 제품 변경 범위가 spec 문서 갱신인 경우에만 허용합니다.
...
- `tasks.md`의 `Plate 상태`가 `Planned`이거나, blocker와 추천 next skill이 명확합니다.
